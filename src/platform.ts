    ...
    override async onStart(reason?: string) {
        ...
        for (const device of devicesResponse.devices) {
            ...
            endpoint.addChildDeviceType('AirQuality', airQualitySensor)
                ...
                .createDefaultTvocMeasurementClusterServer(voc ? voc * 2.2727 : undefined, ConcentrationMeasurement.MeasurementUnit.Ugm3, ConcentrationMeasurement.MeasurementMedium.Air)
                .addRequiredClusterServers();
        ...
        const refreshSensors = async () => {
            ...
            for (const device of airthingsSensors.results) {
                ...
                if (endpoint) {
                    ...
                    if (voc !== undefined) {
                        await airQualityEndpoint?.setAttribute(TotalVolatileOrganicCompoundsConcentrationMeasurement.Cluster.id, 'measuredValue', voc * 2.2727, endpoint.log);
                    }
                }
            }
        };
